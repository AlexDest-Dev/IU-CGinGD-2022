...
void cg::renderer::dx12_renderer::load_pipeline()
{
	...
	//Create swap chain and bind it to window
	...
	//Create descriptor heeap for render target views
...
void cg::renderer::dx12_renderer::load_assets()
{
...
#endif
	result = D3DCompileFromFile(shader_path.wstring().c_str(),
								nullptr,
								nullptr,
								"VSMain",
								"vs_5_0",
	...
	D3D12_INPUT_ELEMENT_DESC input_descriptors[] = {
			...
			{"COLOR", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, 24,
			 D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA,0},
			{"COLOR", 1, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, 36,
			 D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA,0},
			{"COLOR", 2, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, 48,
			 D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA,0},
	...
	for(size_t s = 0; s < model->get_index_buffers().size(); s++)
	{
		...
		vertex_buffers[s]->Map(0, &read_range, reinterpret_cast<void**>(&vertex_data_begin));
...
void cg::renderer::dx12_renderer::populate_command_list()
{
	...
	for(size_t s = 0; s < model->get_vertex_buffers().size(); s++)
	{
		...
		command_list->DrawIndexedInstanced(model->get_index_buffers()[s]->get_number_of_elements(),
										   1, 0, 0, 0);
	}
